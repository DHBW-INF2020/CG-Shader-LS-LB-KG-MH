...
uniform float     iTimeDelta;            // render_pixel time (in seconds)
...
const int SPHERE_COUNT = 10; // Number of spheres to render_pixel
...
vec3 render_pixel(vec2 fragCoord, float windowWidth, float windowHeight, Sphere[SPHERE_COUNT] spheres, Camera camera, Light[LIGHT_COUNT] light){
    vec3 color = vec3(0.0);
#ifdef ANTIALIASING
...
void main(void)
{
    ...
    gl_FragColor = vec4(render_pixel(fragCoord, iResolution.x, iResolution.y, spheres, camera, lights),1.0);
}