...
vec3 backgroundColor = vec3(0.3059, 0.349, 0.4941); // Background color
...
const float WALL_REFLECTIVENESS = 0.07; // Reflectiveness of the walls
...
const float SPHERE_REFLECTIVENESS = 0.2; // Reflectiveness of the spheres
...
struct Test{
    int oClosestSphereIndex;
    vec3 oCollisionPoint;
    bool collided;
};
...
// Outputs the index of the closest sphere and the point of collision.
// Returns true if a collision was found, false otherwise.
Test collideWithClosestTest(Sphere[SPHERE_COUNT] spheres, vec3 rayOrigin, vec3 rayDirection, Test tryMe){
    // Initialize variables to store index and collision time of closest sphere
    int closestSphereIndex;
    float timeOfClosestCollison = -1.0;
    ...
    // If there was a collision with any sphere, set output variables and return true
    if (timeOfClosestCollison != -1.0){
        tryMe.oClosestSphereIndex = closestSphereIndex;
        tryMe.oCollisionPoint = rayOrigin + rayDirection*timeOfClosestCollison;
        tryMe.collided = true;
        return tryMe;
    }
...
vec3 getColor(Sphere[SPHERE_COUNT] spheres, vec3 rayOrigin, vec3 rayDirection, Camera camera, Light light){
    ...
    for (int depth = 0; depth < DEPTH; depth++){
        ...
        Test schmutz = Test(hitSphereIndex,collisionPoint,didCollide);
        /*
        struct Test{
            int oClosestSphereIndex;
            vec3 oCollisionPoint;
        ...
        */
        ...
        schmutz = collideWithClosestTest(spheres, _rayOrigin, _rayDirection,schmutz);
        if (schmutz.collided){
        //if (rayCollided){

            hitSphereIndex = schmutz.oClosestSphereIndex;
            collisionPoint = schmutz.oCollisionPoint;
            ...
            //DEBUG
            //color = vec3(0.098, 0.6863, 0.1647);
        }
    }
...
void main() {
  // Just set the output to a constant reddish-purple
    ...
    float mouseInput = u_mouse.x/100.0;
    float windowRatio = u_resolution.y/u_resolution.x;
    float fov = START_FOV+sin(u_time-mouseInput)*VARIANCE_FOV;
    ...
    float BOUNCINESS_SMALL = abs(JUMP_HEIGHT * sin(u_time));
    float BOUNCINESS_BIG = abs(JUMP_HEIGHT * sin(u_time*1.0/JUMP_RAND));
    float BOUNCINESS_MEDIUM_SMALL = abs(JUMP_HEIGHT * sin(u_time*2.0));
    float BOUNCINESS_MEDIUM_BIG = abs(JUMP_HEIGHT * sin(u_time*1.5));
    ...
    Camera camera = Camera(
        vec3(sin(u_time-mouseInput)*5.0,CAMERA_HEIGHT+cos(u_time)/2.0,cos(u_time-mouseInput)*5.0),
        vec2(sin(u_time/2.0-mouseInput)/8.0-0.15,u_time-mouseInput),
        windowRatio,
    ...
    vec2 pixeclCoord = vec2(gl_FragCoord.x,gl_FragCoord.y);
    vec4 testColor = vec4(render(pixeclCoord, u_resolution.x, u_resolution.y, spheres, camera, lights), 1.0);
    //vec3 render(vec2 fragCoord, float windowWidth, float windowHeight, Sphere[SPHERE_COUNT] spheres, Camera camera, Light[LIGHT_COUNT] light)
    //=============================================================================================================

    
    vec3 color = vec3(0.);
    if(mod(gl_FragCoord.x*100.0,3.0)>0.0)
    {
        color = vec3(0.6784, 0.098, 0.098);
    }
    else
    {
        color = vec3(u_resolution.x, u_resolution.y ,abs(sin(u_time)));
    }
